...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
C.alb_fresh = 0.875;                                                         % albedo fresh snow (fraction)
C.alb_ice = 0.3;                                                           % albedo ice (fraction)
C.alb_firn = 0.60;                                                          % albedo firn (fraction)
...
C.b = 0.4393;                                                               % constant in LWin formulation
C.ecl = 0.9552;                                                             % constant in LWin formulation
C.p = 1.6;                                                                  % exponent in LWin formulation
...
C.rainsnowT = 274.25  ;                                                     % temperature of snow to rain transition (K)
C.Pthres = 2.5d-8;                                                          % threshold precipitation to reset time since last snowfall (m w.e. s-1)
C.cp = 2116;                                                                % specific heat capacity of snow J kg−1 K−1 (source: Steinkogler et al., 2015)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Snow model
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
C.Dfreshsnow = 200.0;                                                       % density of fresh snow (kg m-3) [only used if seasonal snow model is deactivated]
C.Dice = 900.0;                                                             % density of ice (kg m-3)
C.Dfirn = 500.0;                                                            % density of firn (kg m-3)
C.Dwater = 1000.0;                                                          % density of water (kg m-3)
C.Dwind = 350.0;                                                            % density of avalanche deposits (kg m-3)
C.Davalanche = 500.0;                                                       % density of avalanche deposits (kg m-3)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Lapse rates / P gradients (CALIBRATION)
C.precipZ = 0.4 ;                                                           % m we 100 m-1 (calibration parameter)
C.tempZ = - 0.007;                                                          %  K m-1
C.precipZ2d = 0.4 ;                                                         % m we 100m-1 (same as precipZ: for wind + grav input)

% East - West P gradient (optional)
C.ewg = .00 ; % percent per kilometre

% Precipitation gauge undercatch, or non-elevation dependend correction. 
C.p_underC = .038; % ( in %) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
