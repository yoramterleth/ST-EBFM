...
if io.example_run == 0
    ...
    % Precipitation (m w.e.)
    P_annual_sea_level = 30;                                               % Annual precipitation at sea level (m w.e.)
    ...
    CurP =  P_annual_sea_level/52 * (1 + P_z_gradient* ...
            grid.z(:)/100);
   
    wx=180; 
    [CurA]=precipslopebreak(io,CurP,grid,wx,time,WD);
    IN.P(:) = CurA(grid.mask_2D(:)==1); 
    %IN.Ac(:) = CurA(grid.mask_2D(:)==1); % adjust to 1D vector and take away stuff outside glacier
    %IN.Sf(:) = CurP .* (1 + C.precipZ .* NIKKA_d_z/100);
