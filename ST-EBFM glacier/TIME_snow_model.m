...
% Update layer depths & properties, specific to snow contr. source
shift_snowdrift = IN.SD.*C.Dwater./OUT.Dfreshsnow; 
%shift_snowdrift(shift_snowdrift<0)= 0 ; 
shift_avalanches = IN.D .* C.Dwater./C.Davalanche;
shift_snowfall = IN.snow.*C.Dwater./OUT.Dfreshsnow;
shift_riming = OUT.moist_deposition.*C.Dwater./OUT.Dfreshsnow;
shift_all = shift_snowfall + shift_riming + shift_avalanches + shift_snowdrift;

shift_tot(:,1) = shift_all ; 
OUT.surfH(:,1) = OUT.surfH(:,1) + shift_tot(:,1);
runoff_irr_deep = zeros(grid.gpsum,1);
nl = grid.nl;

% Set the density of the avalanche deposit zones (combination of new snowfall
% and avlanched snow)
SDc = IN.SD ; 
SDc(SDc<0) = 0 ; 
prop_avalanches = IN.D ./ (SDc + IN.D + IN.snow) ; 
...
while any(shift_tot>0)
    ...
    OUT.subD(i_noshift,1) = subD_old(i_noshift,1).*subZ_old(i_noshift,1)...
        ./OUT.subZ(i_noshift,1) + ...
        OUT.Dnewsnow(i_noshift,1).*shift(i_noshift,1)./OUT.subZ(...
        i_noshift,1);
    ...
    OUT.subT(i_shift,2) = subT_old(i_shift,1).*subZ_old(i_shift,1)./...
        ...
        ./OUT.subZ(i_shift,2) + ...
        OUT.Dnewsnow(i_shift,1).*(OUT.subZ(i_shift,2)-subZ_old(...
        i_shift,1))./OUT.subZ(i_shift,2);
    OUT.subD(i_shift,1) = OUT.Dnewsnow(i_shift,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% MELT AND SUBLIMATION and erosion 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% calculate ammount of wind erosion 
erosion = -IN.SD ; 
erosion(erosion<0)=0; 
%erosion should only occur if the surface consists of low density snow
erosion(OUT.subD(:,1)> C.Dfirn) = 0 ; 

% Update layer depths & properties
OUT.sumWinit = sum(OUT.subW,2);
mass_removed = (OUT.melt + OUT.moist_sublimation)*1d3 + (erosion .* OUT.subD(:,1)) ; 
...
% OUT.subD(OUT.subD(:)<0.1) = C.Dfreshsnow; 
